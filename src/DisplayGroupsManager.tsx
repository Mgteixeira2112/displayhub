...
const fitLabels: Record<MediaFit, string> = { contain: 'Vídeo inteiro', cover: 'Preencher telas (recomendado)', native: 'Conteúdo preparado' }
...
export default function DisplayGroupsManager() {
  ...
  return <section className="display-groups-module">
    ...
    {selectedGroup && <section className="display-group-editor">
      ...
      {selectedGroup.mode === 'video_wall' && <div className="display-group-mirror-config"><div><p className="eyebrow">Exibição</p><h4>Encaixe da mídia</h4></div><label>Modo<select value={mediaFitDraft} onChange={(event) => void saveMediaFit(event.target.value as MediaFit)} disabled={!canManage || busy}><option value="contain">Vídeo inteiro · pode deixar espaços</option><option value="cover">Preencher telas · recomendado</option><option value="native">Conteúdo preparado · mídia feita para o Video Wall</option></select><small>Para vídeos e imagens comuns, use Preencher telas. Vídeo inteiro preserva toda a mídia e pode deixar espaços. Conteúdo preparado é para material criado na resolução total do Video Wall.</small></label></div>}
