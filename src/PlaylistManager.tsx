...
type PlaylistItem = { id: string; playlist_id: string; source_type: 'content_item' | 'structured_content'; content_item_id: string | null; structured_content_id: string | null; position: number; duration_seconds: number }
...
export default function PlaylistManager({ companyId, role }: Props) {
  ...
  const sourceMap = useMemo(() => new Map(sources.map((source) => [`${source.sourceType}:${source.id}`, source.label])), [sources])
  const playlistMap = useMemo(() => new Map(playlists.map((playlist) => [playlist.id, playlist.name])), [playlists])
  const displayMap = useMemo(() => new Map(displays.map((display) => [display.id, display.name])), [displays])

  const loadAll = useCallback(async () => {
    const [playlistRes, itemRes, contentRes, structuredRes, displayRes, publicationRes] = await Promise.all([
      supabase.from('playlists').select('id,name,description,is_active').order('created_at'),
      supabase.from('playlist_items').select('id,playlist_id,source_type,content_item_id,structured_content_id,position,duration_seconds').order('position'),
    ...
    for (const response of [playlistRes, itemRes, contentRes, structuredRes, displayRes, publicationRes]) if (response.error) throw response.error
  ...
  async function addItem(event: FormEvent) {
    ...
    setBusy(true); setMessage('')
    const payload = { playlist_id: playlistId, company_id: companyId, source_type: sourceType, content_item_id: sourceType === 'content_item' ? id : null, structured_content_id: sourceType === 'structured_content' ? id : null, position, duration_seconds: duration }
    const { error } = await supabase.from('playlist_items').insert(payload)
  ...
  async function createPublication(event: FormEvent) {
    ...
    const { error } = await supabase.from('display_publications').insert({
      ...
      playlist_id: publicationPlaylist,
  ...
  return (
    <section className="workspace-section playlist-workspace">
      <div className="section-heading"><div><p className="eyebrow">Fase 5</p><h2>Playlists e agendamento</h2></div><button className="secondary-button compact" type="button" onClick={() => void loadAll()} disabled={busy}>Atualizar</button></div>
      {message && <p className="form-message playlist-message">{message}</p>}

      {canManage && <div className="playlist-create-grid">
        <form className="content-form" onSubmit={createPlaylist}><h3>Nova playlist</h3><label>Nome<input value={name} onChange={(e) => setName(e.target.value)} required minLength={2} placeholder="Playlist Recepção" /></label><label>Descrição<input value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Conteúdo da TV principal" /></label><button className="primary-button" disabled={busy}>Criar playlist</button></form>
        <form className="content-form" onSubmit={addItem}><h3>Adicionar conteúdo</h3><label>Playlist<select value={playlistId} onChange={(e) => setPlaylistId(e.target.value)} required><option value="">Selecione</option>{playlists.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}</select></label><label>Conteúdo<select value={sourceKey} onChange={(e) => setSourceKey(e.target.value)} required><option value="">Selecione</option>{sources.map((s) => <option key={`${s.sourceType}:${s.id}`} value={`${s.sourceType}:${s.id}`}>{s.label}</option>)}</select></label><div className="playlist-inline"><label>Ordem<input type="number" min="0" value={position} onChange={(e) => setPosition(Number(e.target.value))} /></label><label>Duração (s)<input type="number" min="1" max="3600" value={duration} onChange={(e) => setDuration(Number(e.target.value))} /></label></div><button className="primary-button" disabled={busy}>Adicionar</button></form>
      </div>}

      <div className="playlist-list">{playlists.length === 0 && <p className="empty-state">Nenhuma playlist cadastrada.</p>}{playlists.map((playlist) => <article className="playlist-card" key={playlist.id}><div className="playlist-card-head"><div><strong>{playlist.name}</strong>{playlist.description && <p>{playlist.description}</p>}</div>{canManage && <button className="danger-button" type="button" onClick={() => void remove('playlists', playlist.id)} disabled={busy}>Excluir playlist</button>}</div><div className="playlist-items">{items.filter((item) => item.playlist_id === playlist.id).map((item) => { const sourceId = item.content_item_id || item.structured_content_id || ''; const label = sourceMap.get(`${item.source_type}:${sourceId}`) || 'Conteúdo'; return <div className="playlist-item" key={item.id}><span>#{item.position} · {label}</span><small>{item.duration_seconds}s</small>{canManage && <button type="button" onClick={() => void remove('playlist_items', item.id)}>Remover</button>}</div> })}{items.every((item) => item.playlist_id !== playlist.id) && <small className="empty-state">Playlist vazia.</small>}</div></article>)}</div>

      {canManage && <form className="content-form publication-form" onSubmit={createPublication}><h3>Publicar / agendar em display</h3><div className="publication-grid"><label>Playlist<select value={publicationPlaylist} onChange={(e) => setPublicationPlaylist(e.target.value)} required><option value="">Selecione</option>{playlists.filter((p) => p.is_active).map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}</select></label><label>Display<select value={displayId} onChange={(e) => setDisplayId(e.target.value)} required><option value="">Selecione</option>{displays.filter((d) => d.is_active && !d.revoked_at).map((d) => <option key={d.id} value={d.id}>{d.name}</option>)}</select></label><label>Início opcional<input type="datetime-local" value={startsAt} onChange={(e) => setStartsAt(e.target.value)} /></label><label>Fim opcional<input type="datetime-local" value={endsAt} onChange={(e) => setEndsAt(e.target.value)} /></label><label>Repetição<select value={repeatMode} onChange={(e) => setRepeatMode(e.target.value as 'always' | 'daily')}><option value="always">Contínua</option><option value="daily">Horário diário</option></select></label>{repeatMode === 'daily' && <><label>De<input type="time" value={dailyStart} onChange={(e) => setDailyStart(e.target.value)} required /></label><label>Até<input type="time" value={dailyEnd} onChange={(e) => setDailyEnd(e.target.value)} required /></label></>}</div>{repeatMode === 'daily' && <div className="weekday-row">{week.map(([value,label]) => <label key={value}><input type="checkbox" checked={weekdays.includes(value)} onChange={(e) => setWeekdays(e.target.checked ? [...weekdays, value].sort() : weekdays.filter((day) => day !== value))} />{label}</label>)}</div>}<button className="primary-button" disabled={busy || (repeatMode === 'daily' && weekdays.length === 0)}>Criar programação</button></form>}
